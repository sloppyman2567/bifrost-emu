...
[![Version: 1.4.0-alpha.2](https://img.shields.io/badge/version-1.4.0--alpha.3-orange.svg)](CHANGELOG.md)
...
| `toybox-aarch64 sh` | ❌ Hangs | sh's signal-pending polling loop waits for SIGCHLD from a forked child that never existed (our `clone()` returns 0 = "you are the child"). Tracked for v1.4.0-alpha.3. |
...
When the host OS delivers a signal to the emulator process (SIGINT
from Ctrl-C, SIGTERM from `kill`, SIGCHLD when a host child exits,
SIGWINCH on terminal resize, etc.), a host `sigaction` handler now
queues the signal number. The run loop drains the queue every ~4K
instructions and calls `deliver_signal()` for each entry, which
invokes the guest's installed handler (or applies the default
disposition). Previously, signals sent to the emulator only affected
the host — the guest never saw them, and signal-wait loops hung
forever. Forwarded: SIGHUP, SIGINT, SIGQUIT, SIGUSR1/2, SIGPIPE,
SIGALRM, SIGTERM, SIGCHLD, SIGCONT, SIGTSTP, SIGTTIN, SIGTTOU,
SIGURG, SIGXCPU, SIGXFSZ, SIGVTALRM, SIGPROF, SIGWINCH, SIGIO. NOT
forwarded (synchronous): SIGSEGV, SIGBUS, SIGFPE, SIGILL, SIGTRAP,
SIGABRT, SIGSYS.

**New syscalls (unblocks toybox)**

- `ppoll` (73) — previously missing; toybox `sh` uses this to wait
  for stdin input. Forwards to host `poll(2)`.
- `fallocate` (47) — toybox `sh` hits this during line-edit setup.
  Passes through to host for `mode=0`.
- `umask` (166) — toybox `sh` calls `umask(0)` during init. Passes
  through to host.
- `wait4` (260), `waitid` (272), `waitpid` (247) — return `-ECHILD`
  so guests that fork via `clone()` without `CLONE_VM` don't loop
  forever waiting for a child that never existed.

**Critical syscall numbering fix**

`rt_sigsuspend` (syscall 133) was misimplemented as `rt_sigreturn`.
The real `rt_sigreturn` is syscall 139. Our code had `case 133`
handling `rt_sigreturn` (wrong) and no handler for 139. Any guest
calling `sigsuspend()` got a signal-frame pop instead of a blocking
wait, corrupting CPU state. Now `case 133` calls host `sigsuspend`
and `case 139` pops the signal frame. The trampoline code (which
uses `mov x8, #139`) was already correct — only the dispatcher was
wrong.

**`clone()` / `fork()` semantics**

`clone()` without `CLONE_VM` (i.e. `fork`) now returns 0 (treat as
child), matching `vfork()` semantics. Previously returned `-ENOSYS`,
which broke any guest using `fork()`. Real fork support (with
copy-on-write guest memory) is a future v1.4.x item.

**Decode cache: 2-way set-associative**

Upgraded from direct-mapped to 2-way set-associative (8192 sets × 2
ways = 16384 entries, ~1.5 MB per vCPU). Direct-mapped caches suffer
conflict misses when two hot PCs hash to the same set. 2-way reduces
conflict misses at the cost of one extra tag compare on the hot path.
LRU replacement via a packed bitset. Total cache size unchanged
(16384 entries); the improvement is in the hit rate for adversarial
access patterns.

**Benchmarking**

New `bench.sh` script runs a representative workload mix (fib, cat,
wc, head, sort, rev, toybox echo/uname/whoami/sleep/true) through
the emulator and reports instruction count, wall time, MIPS, and
decode-cache hit rate for each. Supports both interpreter (default)
and `--jit` modes. Example output:

```
BENCHMARK                              INSTRS    TIME(s)       MIPS     CACHE%
fib(30) [assembled]                  3447      0.000      15.67       69.7
fib(40) musl-static                  3548      0.000      16.84       70.4
wc README.md                      1332336      0.027      49.34       99.8
toybox echo hi                      21309      0.001      26.98       88.2
toybox uname                        25091      0.001      28.19       91.1
```
...
- **Signal delivery is partial.** `rt_sigaction` installs handlers,
  ...
  signals to the current thread, and host signals (SIGINT/SIGTERM/
  SIGCHLD/etc.) are forwarded to the guest via a host `sigaction`
  handler + run-loop drain (v1.4.0-alpha.1). However: no
  `siginfo_t`/`ucontext_t` contents are passed to handlers, no
  `SA_RESTART`, no signal masks, no `sigaltstack`, no real-time
  signals 32+, and no cross-thread delivery. See `signal.hpp` for
  the full list.

- **`fork()` is stubbed.** `clone()` without `CLONE_VM` returns 0
  (treat as child), matching `vfork()` semantics. Real fork support
  (with copy-on-write guest memory) is a future v1.4.x item. As a
  ...
  commands. Tracked for v1.4.0-alpha.3.
...
1. Stabilize. No new features — just bug fixes from the alpha.3
