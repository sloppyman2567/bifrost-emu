...
// per-chunk mesh sequence number (bumped in chunkmesh_mesh) for diagnosing
// which chunk triggers a mesh-buffer overflow.
int mesh_gen_counter = 0;
...
#define APPEND(_type, _buffer, _v) ({ \
        size_t _cap_ele_ = (_buffer)->capacity / sizeof(_type); \
        if ((_buffer)->elements >= _cap_ele_) { \
            fprintf(stderr, "[MESH-OVF] buf=%d elems=%zu cap_elems=%zu index=%zu chunk_gen=%d\n", \
                    (_buffer)->type, (_buffer)->elements, _cap_ele_, (_buffer)->index, mesh_gen_counter); \
        } else { \
