    ...
    if (!heightmap->flags.generated) {
        ...
        for (s64 x = 0; x < CHUNK_SIZE.x; x++) {
            for (s64 z = 0; z < CHUNK_SIZE.z; z++) {
                ...
                if (x == 0 && z == 0) {
                    fprintf(stderr, "[DBG-N] h=%f m=%f t=%f r=%f n=%f p=%f biome=%d h_b=%f\n",
                            (double)h, (double)m, (double)t, (double)r, (double)n, (double)p,
                            (int)biome_id,
                            (double)(((h * 32.0f) + (n * 256.0f)) * biome.scale + (biome.roughness * r * 2.0f)));
                }
...
#define WG_GET_H(_x, _z)\
    heightmap->worldgen_data[\
        ...
        for (s64 x = 0; x < CHUNK_SIZE.x; x++) {
            for (s64 z = 0; z < CHUNK_SIZE.z; z++) {
                ...
                WG_GET_H(x, z).h = v;
                if (x == 0 && z == 0) {
                    fprintf(stderr, "[DBG-SM] v=%f h_after=%lld h_b00=%f h_b10=%f h_b01=%f h_b11=%f b_after=%lld\n",
                            (double)v, (long long)WG_GET_H(0,0).h,
                            (double)WG_GET_H(0,0).h_b, (double)WG_GET_H(1,0).h_b,
                            (double)WG_GET_H(0,1).h_b, (double)WG_GET_H(1,1).h_b,
                            (long long)WG_GET_H(0,0).b);
                }
            }
        }
    }

    struct WorldgenData wg0 = heightmap->worldgen_data[0];
    fprintf(stderr, "[DBG-GEN] pos=(%ld,%ld,%ld) seed=%lu h00=%f h00i=%lld h_b00=%f b00=%lld\n",
            (long)chunk->position.x, (long)chunk->position.y, (long)chunk->position.z,
            chunk->world->seed,
            wg0.h,
            (long long)wg0.h,
            wg0.h_b,
    ...
    long cnt_water = 0, cnt_top = 0, cnt_under = 0, cnt_stone = 0, cnt_air = 0;
    ...
    for (s64 x = 0; x < CHUNK_SIZE.x; x++) {
        for (s64 z = 0; z < CHUNK_SIZE.z; z++) {
            ...
            for (s64 y = 0; y < CHUNK_SIZE.y; y++) {
                ...
                if (block == WATER) cnt_water++;
                else if (block == AIR) cnt_air++;
                else if (block == top_block) cnt_top++;
                else if (block == under_block) cnt_under++;
                else cnt_stone++;
    ...
    fprintf(stderr, "[DBG-GEN] done pos.y=%ld water=%ld air=%ld top=%ld under=%ld stone=%ld\n",
            (long)chunk->position.y, cnt_water, cnt_air, cnt_top, cnt_under, cnt_stone);
}