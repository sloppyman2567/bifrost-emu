...
void sky_render(struct Sky *self) {
    ...
    fprintf(stderr,
        "[SKY-DBG] state.ticks=%llu world.ticks=%llu state=%d progress=%f\n"
        "[SKY-DBG]   fog=(%f,%f,%f,%f) sky=(%f,%f,%f,%f) void=(%f,%f,%f,%f) cloud=(%f,%f,%f,%f)\n"
        "[SKY-DBG]   rend.clear=(%f,%f,%f,%f) cam=(%f,%f,%f)\n",
        (unsigned long long)state.ticks, (unsigned long long)self->world->ticks,
        (int)self->state, (double)self->sky_state_progress,
        fog_color.x, fog_color.y, fog_color.z, fog_color.w,
        sky_color.x, sky_color.y, sky_color.z, sky_color.w,
        void_color.x, void_color.y, void_color.z, void_color.w,
        cloud_color.x, cloud_color.y, cloud_color.z, cloud_color.w,
        state.renderer.clear_color.x, state.renderer.clear_color.y,
        state.renderer.clear_color.z, state.renderer.clear_color.w,
