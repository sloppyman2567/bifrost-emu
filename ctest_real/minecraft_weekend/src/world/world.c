...
void world_init(struct World *self) {
    ...
    // v1.5.x.alpha: tiny test world so a full CPU-emulated frame completes
    // in seconds (4096 chunks was minutes of emulated world-gen per frame).
    self->chunks_size = 4;
...
void world_render(struct World *self) {
    ...
    // render solid geometry in no particular order
    {
        ...
        for (size_t i = 0; i < NUM_CHUNKS(self); i++) {
            struct Chunk *cc = self->chunks[i];
            if (cc == NULL || cc == (void*)INT64_MAX) continue;
            loaded++;
            if (cc->mesh == NULL) continue;
            if (cc->mesh->flags.dirty) ;
            base_idx += (int)cc->mesh->indices.base.count;
            trans_idx += (int)cc->mesh->indices.transparent.count;
            blocks += cc->count;
            if (cc->flags.empty) empty++;
            for (int y = 0; y < CHUNK_SIZE.y; y++)
            for (int x = 0; x < CHUNK_SIZE.x; x++)
            for (int z = 0; z < CHUNK_SIZE.z; z++) {
                enum BlockId b = chunk_data_to_block(chunk_get_block(cc, (ivec3s){{x,y,z}}));
                if (b == WATER) water++;
                else if (b == STONE) stone++;
