...
public:
    ...
    Memory() {
        // Allocate a 4GB direct-access window for the JIT. This is a
        // lazy mmap — Linux only allocates physical pages on first
        // access (demand paging). The window is PROT_READ|PROT_WRITE.
        void* p = mmap(nullptr, DIRECT_WINDOW_SIZE,
                       PROT_READ | PROT_WRITE,
                       MAP_PRIVATE | MAP_ANONYMOUS | MAP_NORESERVE, -1, 0);
        if (p != MAP_FAILED) {
            direct_window_ = (uint8_t*)p;
        }
    }
    ...
    void map_range(uint64_t addr, uint64_t size) {
        ...
        for (; start < end; start += PAGE_SIZE) {
            ...
            // For addresses in the direct window (< 4GB), the window
            // IS the storage — no need to create a pages_ entry. The
            // window pages are zero-initialized by mmap.
            if (direct_window_ && start < DIRECT_WINDOW_SIZE) {
                // Page is already accessible in the window (mmap'd).
                // No action needed.
                continue;
            }
            // For addresses >= 4GB, use the sparse pages_ map.
    ...
    bool is_mapped(uint64_t addr, uint64_t size) const {
        ...
        // Direct window: pages < 4GB are always "mapped" (they're
        // backed by the mmap'd window, zero-initialized on access).
    ...
    void write(uint64_t addr, const void* src, size_t n, PageCache* pc = nullptr) {
        if (n == 0) return;
        // Fast path: direct window for addresses < 4GB.
    ...
    uint64_t mmap_alloc(uint64_t size, uint64_t hint = 0) {
        ...
        // Track the allocation so mremap can detect collisions.
        // We store the page-aligned size so it matches the actual
        // bump-pointer consumption above.
        allocations_[base] = aligned_size;
        return base;
    }

    // ── mremap growth with collision detection ───────────────────────
    //
    // When musl's mallocng grows a "big" allocation via mremap, it
    // passes MREMAP_MAYMOVE and is prepared for the address to change.
    // The previous implementation always grew in place by calling
    // map_range(extra_start, extra_end - extra_start), which is a
    // no-op on already-mapped pages. This silently corrupted other
    // allocations that had been placed in those pages by subsequent
    // mmap calls (because the bump pointer tightly packs allocations).
    //
    // The fix: track every mmap'd region in `allocations_`. On mremap
    // growth, check whether the range [old_addr+old_size, new_size)
    // overlaps with any other tracked allocation. If it does, allocate
    // a fresh region of new_size bytes, copy old_size bytes from the
    // old address, and return the new address (musl handles the move).
    // If it doesn't, grow in place and update the recorded size.
    //
    // Returns the (possibly new) address of the resized mapping.
    uint64_t mremap_grow(uint64_t old_addr, uint64_t old_size,
                         uint64_t new_size) {
        ...
        // — the guest won't access them, and we never reclaim address
        // space anyway (bump allocator).
        ...
        // Grow: check for collision with the pages just past the
        // current allocation's end. We do this under the same lock
        // that protects pages_ and allocations_ so a concurrent mmap
        // can't slip in between our check and our decision.
        ...
        {
            std::lock_guard<std::mutex> g(mu_);
            // If any other tracked allocation starts inside
            // [extra_start, extra_end), we can't grow in place.
            for (const auto& kv : allocations_) {
                ...
                // Overlap test: ranges [extra_start, extra_end) and
                // [other_base, other_end) overlap iff
                //   extra_start < other_end && other_base < extra_end.
        ...
        if (can_grow_in_place) {
            // Map the additional pages (no-op if already present, e.g.
            // inside the direct window) and update the recorded size.
        ...
        // Collision detected: allocate a fresh region, copy the data,
        // and return the new address. The old pages stay mapped (our
        // munmap is a no-op), which is safe — musl's metadata will be
        // updated to point at the new address, and the old pages are
        // simply leaked (no different from any other unmapped region
        // in our sparse model).
        uint64_t new_addr = mmap_alloc(new_size, 0);
        // Copy old_size bytes (not old_aligned — the guest's data may
        // not extend into the alignment padding).
        if (old_size > 0) {
            std::vector<uint8_t> buf(old_size);
            read(old_addr, buf.data(), old_size);
            write(new_addr, buf.data(), old_size);
        }
        // Remove the old allocation from our tracking so a future
        // mremap on the same address doesn't get confused. The pages
        // themselves stay mapped.
        {
            std::lock_guard<std::mutex> g(mu_);
    ...
    // Record a foreign allocation (one made outside mmap_alloc, e.g.
    // by direct map_range calls from the brk handler or ELF loader).
    // This lets mremap_grow see those regions when checking for
    // collisions. Currently unused but kept for future use.
    void track_allocation(uint64_t addr, uint64_t size) {
        std::lock_guard<std::mutex> g(mu_);
        uint64_t aligned = (size + PAGE_MASK) & ~PAGE_MASK;
        allocations_[addr] = aligned;
    }

    // Remove an allocation from tracking (used by munmap to keep the
    // map accurate). The pages themselves stay mapped (we never
    // reclaim them in this sparse model).
    ...
    //
    // The window covers the low 4GB of the guest address space. Guest
    // pages in this range are stored DIRECTLY in the window (not in
    // pages_). The interpreter's read()/write() also use the window
    // for addresses < 4GB, so there's no sync issue — the window IS
    // the storage. pages_ is only used for addresses >= 4GB (stack,
    // high mmap region).
    static constexpr uint64_t DIRECT_WINDOW_SIZE = 4ULL * 1024 * 1024 * 1024;  // 4 GB
    uint8_t* direct_window_ = nullptr;

    uint8_t* direct_window() const { return direct_window_; }

    // Map a guest page into the direct window. Called by map_range().
    void map_direct(uint64_t page_num, std::vector<uint8_t>& page) {
        uint64_t guest_addr = page_num * PAGE_SIZE;
        if (guest_addr >= DIRECT_WINDOW_SIZE) return;
        if (!direct_window_) return;
        // Copy the page data into the window. After this, the window
        // is the primary storage for this page — the vector in pages_
        // is no longer used for reads/writes (they go to the window).
        memcpy(direct_window_ + guest_addr, page.data(), PAGE_SIZE);
    }

    // Check if a guest address is in the direct window.
...
private:
    ...
    // in-place growth would collide with a later allocation (which
    // would silently corrupt that allocation's pages).
...
public:
    ...
    // entries (~1.5 MB per vCPU). This is the hot path — ~100% hit
    // rate for tight loops.
    //
    // v1.4.0-alpha.1: upgraded from direct-mapped to 2-way
    // set-associative. Direct-mapped caches suffer conflict misses
    // when two hot PCs hash to the same set (e.g., a tight loop body
    // at PC A and a frequently-called helper at PC B where A and B
    // differ by a multiple of the cache size). 2-way reduces conflict
    // misses at the cost of one extra tag compare on the hot path.
    // The hash uses bits [15:2] of the PC (14 bits = 16384 sets,
    // shifted by 2 to ignore the low 2 instruction-alignment bits).
    ...
    // Packed into a bitset for cache density.
    std::vector<uint8_t> decode_cache_lru = std::vector<uint8_t>((DECODE_CACHE_SETS + 7) / 8, 0);
...
public:
    ...
    int run() {
        ...
        while (main_cpu_.running) {
            ...
            // Drain the host-signal queue every ~4K instructions. This
            // is frequent enough that signal-wait loops (like toybox
            // sh's `sig_process_pending()`) see delivered signals
            // within a few iterations, but rare enough that the lock
            // acquire is negligible overhead (~0.025% of instructions).
            // When a signal is delivered, the guest's signal handler
            // runs by jumping to it (PC = handler address), so we
            // don't need to do anything special to "return" from the
            // handler — the trampoline + rt_sigreturn syscall handle
            // that.
            if ((count & 0xFFF) == 0) {
    ...
    // v1.4.0-alpha.1: install host signal handlers that forward
    // SIGINT/SIGTERM/SIGCHLD/etc. to the guest. Called once from
    // main.cpp before run().
    ...
    // Public syscall wrapper — used by the IR executor (ops.cpp) and the
    // JIT (frostjit.cpp) for SVC handling. Routes to the private syscall().
    void syscall_public(CPU& cpu) { syscall(cpu); }

    // Public accessor for the main CPU (used by external debug tools).
...
private:
    ...
    // ── Host-to-guest signal forwarding (v1.4.0-alpha.1) ─────────────
    // When the host OS sends a signal to the emulator process (e.g.,
    // SIGINT from Ctrl-C, SIGTERM from `kill`, SIGCHLD when a host
    // child exits), our host signal handler pushes the signal number
    // onto this queue. The run loop drains the queue between
    // instructions and calls deliver_signal() for each entry, which
    // invokes the guest's installed handler (if any) or applies the
    // default disposition (e.g., terminate on SIGINT/SIGTERM).
    //
    // Without this, signals sent to the emulator only affect the host
    // process — the guest never sees them, and signal-wait loops
    // (like toybox sh's `sig_process_pending()`) hang forever.
    ...
    bool drain_host_signals(CPU& cpu);  // returns true if any were delivered
    ...
    void step(CPU& cpu) {
        if (trace_) {
            fprintf(stderr, "[trace tid=%d] pc=0x%08llx x0=0x%llx x1=0x%llx x2=0x%llx x3=0x%llx x4=0x%llx x5=0x%llx x8=0x%llx x16=0x%llx x17=0x%llx x19=0x%llx x20=0x%llx x21=0x%llx x22=0x%llx x23=0x%llx x24=0x%llx x25=0x%llx x26=0x%llx x27=0x%llx x28=0x%llx x29=0x%llx x30=0x%llx pstate=0x%x sp=0x%llx v0lo=0x%llx v0hi=0x%llx v1lo=0x%llx v1hi=0x%llx v2lo=0x%llx v2hi=0x%llx\n",
                    cpu.tid,
                    ...
                    (unsigned long long)cpu.regs[16],
