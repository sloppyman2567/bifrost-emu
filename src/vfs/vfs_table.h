...
// ── AudioVNode: /dev/dsp, /dev/snd, /dev/audio wrapper ─────────────────
// Delegates PCM writes to the Audio backend, which buffers them and can
// dump to a WAV file on close. Reads return 0 (recording not supported).
class Audio;

class AudioVNode : public VNode {
public:
    explicit AudioVNode(Audio* audio, int flags)
    ...
    Audio* audio_backend() const { return audio_; }
